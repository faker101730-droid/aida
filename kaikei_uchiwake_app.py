...
def parse_file(uploaded_file):
    ...
    fukushi_candidate_cols = []
    for key in ["日付","借方中区分","貸方中区分","借方金額","貸方金額","摘要文"]:
        fukushi_candidate_cols.append(COLUMN_ALIASES[key][0])
    fukushi_row = detect_header_row(raw, fukushi_candidate_cols)

    candidates = []
    for kind, row in [("initial", init_row), ("journal_standard", standard_row), ("journal_fukushi", fukushi_row)]:
        try:
            df = normalize_columns(read_any_table(uploaded_file, header=row))
            detected = detect_file_kind(df)
            candidates.append((kind, row, detected, df))
        except Exception:
            pass

    # 正式判定優先
    for _, _, detected, df in candidates:
        if detected == "initial":
            return "initial", df
    for _, _, detected, df in candidates:
        if detected in ["journal_standard", "journal_fukushi"]:
            return "journal", df
...
def prepare_journal(df):
    ...
    if kind == "journal_fukushi":
        ...
        out["借方科目"] = [choose_account_label(m, s, b) for m, s, b in zip(out["借方中区分"], out["借方小区分"], out["借方補助区分"])]
        out["貸方科目"] = [choose_account_label(m, s, b) for m, s, b in zip(out["貸方中区分"], out["貸方小区分"], out["貸方補助区分"])]
...
def choose_accounts(initial_df, journal_df, manual_text):
    ...
    all_accounts = sorted(
        set(initial_df["勘定科目"].dropna().tolist())
        | set(journal_df["借方科目"].dropna().tolist())
        | set(journal_df["貸方科目"].dropna().tolist())
    )
    if manual:
...
def to_excel(summary, current_detail, history_detail, account, month_text):
    ...
    output = io.BytesIO()
    cond = pd.DataFrame({"項目":["対象月","勘定科目"], "内容":[month_text, account]})
...
st.caption("福祉の森の仕訳日記帳CSVをそのまま読み込める版。列名ゆらぎも吸収します。")
...
if init_file and journal_file:
    try:
        ...
        st.markdown(f'<div class="info-box">基準日: {init_base.date()} / 対象月: {target_month} / 勘定科目: {account}</div>', unsafe_allow_html=True)
